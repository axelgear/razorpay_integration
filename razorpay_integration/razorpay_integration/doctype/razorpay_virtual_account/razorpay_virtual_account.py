...
class RazorpayVirtualAccount(Document):
    def validate(self):
        ...
        if self.receiver_types and self.receiver_types not in ["bank_account", "qr_code", "tpv"]:
            frappe.throw("Receiver Types must be one of: bank_account, qr_code, tpv.")
    ...
    def create_virtual_account(self):
        ...
        va_data = {
            "receivers": {
                "types": [self.receiver_types] if self.receiver_types else ["bank_account"]
            },
        ...
        try:
            ...
            self.virtual_account_id = settings.virtual_account_prefix + va["id"]
